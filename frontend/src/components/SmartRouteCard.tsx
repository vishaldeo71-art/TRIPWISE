...
export default function SmartRouteCard({
  ...
  return (
    <div className="glass-card rounded-2xl p-5 border border-white/[0.07] hover:border-white/[0.12] transition-all">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-start space-x-3.5">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <h4 className="text-xs font-extrabold uppercase tracking-wider text-slate-300">Smart Route Optimization</h4>
              {optimizedApplied && (
        ...
        <button
          ...
          className="flex items-center space-x-2 text-xs px-4 py-2.5 rounded-xl bg-slate-900/90 hover:bg-slate-800 text-slate-100 font-semibold border border-slate-800 transition-all hover:scale-[1.02] active:scale-[0.98] shrink-0 justify-center"
        >
          <NavigationIcon size={14} className="text-sky-400" />
      ...
      {optimization && (
        <div className="mt-4 pt-4 border-t border-white/[0.06] animate-fade-in">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs mb-3">
            <div className="bg-slate-950/60 rounded-xl p-2.5 border border-slate-800/80 text-center sm:text-left">
              <span className="text-slate-400 block text-[10px] uppercase font-bold tracking-wider">Stops Analyzed</span>
              <span className="text-white font-extrabold mt-0.5 block">
                {optimization.locationCount} locations
              </span>
            </div>
            <div className="bg-slate-950/60 rounded-xl p-2.5 border border-slate-800/80 text-center sm:text-left">
              <span className="text-slate-400 block text-[10px] uppercase font-bold tracking-wider">Route Distance</span>
              <span className="text-white font-extrabold mt-0.5 block">
                {optimization.optimizedDistanceKm} km
              </span>
            </div>
            <div className="bg-slate-950/60 rounded-xl p-2.5 border border-slate-800/80 text-center sm:text-left">
              <span className="text-slate-400 block text-[10px] uppercase font-bold tracking-wider">Distance Saved</span>
              <span className="text-emerald-300 font-extrabold mt-0.5 block">
                ~{optimization.distanceSavedKm} km saved
              </span>
            </div>
            <div className="bg-slate-950/60 rounded-xl p-2.5 border border-slate-800/80 text-center sm:text-left">
              <span className="text-slate-400 block text-[10px] uppercase font-bold tracking-wider">Est. Travel Time</span>
              <span className="text-sky-300 font-extrabold mt-0.5 block">
                ~{optimization.estimatedTravelTimeMins} mins
              </span>
            </div>
          </div>

          <div className="flex items-center gap-2 text-[11px] text-emerald-300 bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-2.5">
