    ...
    <div className="glass-card rounded-2xl p-5 border border-white/[0.07] hover:border-white/[0.12] transition-all">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start space-x-3.5">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <h4 className="text-xs font-extrabold uppercase tracking-wider text-slate-300">Weather Advisor</h4>
              <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold border ${badgeColor}`}>
                {advisorTitle}
              </span>
            </div>
            <p className="text-xs text-slate-300 mt-1 max-w-xl leading-relaxed">{advisorMessage}</p>
          </div>
        </div>
        ...
        {onAskAI && (
          <button
            onClick={onAskAI}
            className="hidden sm:flex items-center space-x-1.5 text-xs px-3.5 py-2 rounded-xl bg-slate-900/90 hover:bg-slate-800 text-amber-300 hover:text-amber-200 border border-slate-800 transition-colors shrink-0 font-semibold"
          >
      ...
      <div className="grid grid-cols-3 gap-3 mt-4 pt-3 border-t border-white/[0.06] text-xs">
        <div className="bg-slate-950/60 rounded-xl p-2.5 text-center border border-slate-800/80">
          <span className="text-slate-400 block text-[10px] uppercase font-bold tracking-wider">Temperature</span>
          <span className="text-slate-100 font-extrabold mt-0.5 block">{tempC}°C</span>
        </div>
        <div className="bg-slate-950/60 rounded-xl p-2.5 text-center border border-slate-800/80">
          <span className="text-slate-400 block text-[10px] uppercase font-bold tracking-wider">Rain Risk</span>
          <span className="text-slate-100 font-extrabold mt-0.5 block">{rainProb}%</span>
        </div>
        <div className="bg-slate-950/60 rounded-xl p-2.5 text-center border border-slate-800/80">
          <span className="text-slate-400 block text-[10px] uppercase font-bold tracking-wider">Condition</span>
          <span className="text-slate-100 font-extrabold mt-0.5 block truncate">{condition}</span>
        </div>
      </div>
    </div>
  );
